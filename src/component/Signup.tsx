...
import { signupCheckType } from "@dewang002/medium";
import axios from "axios";
import { Backend_URL } from "../config";

const Signup = () => {
  const [inputData, setInputData] = useState<signupCheckType>({
    name: "",
    email: "",
    password: "",
  });
  ...
  const sendRequest = async ()=>{
    try{
      const res = await axios.post(`${Backend_URL}/api/v1/user/signup`,inputData)
      if(res){
        setInputData({
          name:"",
          email:"",
      ...
      console.log("here sign up problem")
    }
  }
